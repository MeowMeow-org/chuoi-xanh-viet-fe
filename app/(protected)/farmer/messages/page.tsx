...
export default function FarmerMessagesPage() {
  ...
  const activeConv = searchParams.get("chat");

  const active = convList.find(
  ...
  if (active) {
    return (
      <div className="flex h-dvh flex-col bg-[hsl(120,20%,98%)]">
        <div className="flex items-center gap-3 border-b bg-white px-4 py-3">
          <Button
            variant="ghost"
            size="icon"
            className="h-8 w-8"
            onClick={() => router.replace("/farmer/messages")}
          >
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4 px-4 py-4 pb-20 sm:px-6 md:pb-8">
      ...
      {convList.length === 0 ? (
        <p className="py-8 text-center text-sm text-[hsl(150,7%,45%)]">
        ...
        <div className="space-y-2">
          {convList.map((conversation: Conversation) => (
            <Card
              ...
              onClick={() =>
                router.replace(`/farmer/messages?chat=${conversation.id}`)
              }
            >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-[hsl(150,7%,45%)]">
                      {formatDateDDMMYYYY(conversation.lastTimestamp)}
                    </span>
                  </div>
                  <p className="truncate text-sm text-[hsl(150,7%,45%)]">
